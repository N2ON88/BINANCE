...
{"approved":true/false,"reason":"한국어 1-2문장 근거","confidence":0-100}`;
  ...
  try {
    ...
    if (data.error) {
      ...
      addAutoLog(`⚠️ AI확인 실패(${data.error.message?.slice(0,40)}) - ${strict?'스킵(strict)':'로컬판단 진입'}`);
    ...
    let verdict;
    try {
      verdict = JSON.parse(txt.replace(/```json|```/g,'').trim());
    } catch(parseErr) {
      const strict = autoState.aiStrict;
    ...
    const strict = autoState.aiStrict;
    addAutoLog(`⚠️ AI확인 오류(${e.message?.slice(0,40)}) - ${strict?'스킵(strict)':'로컬판단 진입'}`);
...
app.post("/api/auto/config", (req, res) => {
  const { enabled, interval, symbols, leverage, marginUsdt, minConfidence, aiConfirm, aiGateScore, cooldownMin, aiStrict } = req.body;
