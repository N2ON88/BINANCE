...
const TG_TOKEN       = process.env.TELEGRAM_TOKEN     || ""; // 텔레그램 봇 토큰
const TG_CHAT_ID     = process.env.TELEGRAM_CHAT_ID   || ""; // 알림 받을 채팅 ID
...
let autoState = {
  ...
  aiModel: "claude-sonnet-4-6", // 2차 확인 모델 (sonnet=정확, haiku=저비용)
...
// ── 텔레그램 알림 (진입/청산/오류) ────────────────────────────────────────────
// 비동기 fire-and-forget: 알림 실패가 매매 흐름을 막지 않도록 await 안 함
function notifyTelegram(text) {
  if (!TG_TOKEN || !TG_CHAT_ID) return; // 미설정 시 조용히 무시
  ...
  fetch(`https://api.telegram.org/bot${TG_TOKEN}/sendMessage`, {
    ...
    .then(r => r.json())
    .then(d => { if (!d.ok) console.error("⚠️ 텔레그램 전송 실패:", d.description || ""); })
    .catch(e => console.error("⚠️ 텔레그램 오류:", e.name === "AbortError" ? "타임아웃" : e.message))
    .finally(() => clearTimeout(t));
}

// ── 심볼별 쿨다운 / 포지션 모드 캐시 ──────────────────────────────────────────
const lastTradeAt = {};            // { BTCUSDT: timestamp } 마지막 진입/청산 시각
const knownPositions = {};         // { BTCUSDT: {amt, entry} } 직전 사이클에 열려있던 포지션 (SL/TP 체결 감지용)
...
{"approved":true/false,"reason":"근거","confidence":0-100}`;
  ...
  // 허용 모델만 사용 (오타/잘못된 값 방어)
  const ALLOWED_MODELS = ["claude-sonnet-4-6", "claude-haiku-4-5", "claude-opus-4-8"];
  const model = ALLOWED_MODELS.includes(autoState.aiModel) ? autoState.aiModel : "claude-sonnet-4-6";
...
async function runAutoTrade() {
  ...
  for (const symbol of autoState.symbols) {
    try {
      ...
      // SL/TP 체결 감지: 직전 사이클엔 포지션이 있었는데 지금 없으면 바이낸스에서 청산된 것
      const curAmt = sig.activePos ? parseFloat(sig.activePos.positionAmt) : 0;
      if (knownPositions[symbol] && curAmt === 0) {
        addAutoLog(`📭 ${symbol}: 포지션 종료 감지 (SL/TP 체결 또는 외부 청산)`);
        notifyTelegram(`🔔 포지션 종료 ${symbol}\nSL/TP 체결 또는 외부 청산으로 포지션이 닫혔습니다.\n${nowStr()}`);
        delete knownPositions[symbol];
        // 잔여 주문 정리
        try { await binanceFetch("/fapi/v1/allOpenOrders","DELETE",{symbol},true); } catch(e){}
      } else if (curAmt !== 0) {
        knownPositions[symbol] = { amt: curAmt, entry: parseFloat(sig.activePos.entryPrice||0) };
      }
      ...
      knownPositions[symbol] = { amt: side==="BUY"?qty:-qty, entry: price };  // SL/TP 체결 감지용 등록

      addAutoLog(`✅ ${sig.signal} ${symbol} x${autoState.leverage} qty:${qty} @ $${roundPrice(symbol,price)} | 불:${sig.bullScore||0}/베어:${sig.bearScore||0}`);
      notifyTelegram(`🟢 진입 ${sig.signal} ${symbol}\n레버리지 x${autoState.leverage} · 수량 ${qty}\n진입가 $${roundPrice(symbol,price)}\n점수 불:${sig.bullScore||0}/베어:${sig.bearScore||0}\n${nowStr()}`);
      ...
      if(sig.stopLoss && !slOk) {
        try {
          ...
          notifyTelegram(`⚠️ 롤백 ${symbol}\nSL 설정 실패로 진입 직후 포지션을 즉시 청산했습니다.\n${nowStr()}`);
        } catch(e){ addAutoLog(`🚨 ${symbol}: 롤백 청산 실패! 수동확인 필요: ${e.message}`); notifyTelegram(`🚨 긴급 ${symbol}\n롤백 청산까지 실패했습니다. SL 없는 포지션이 남아있을 수 있으니 즉시 수동 확인하세요!\n사유: ${e.message}\n${nowStr()}`); }
        continue;
      }
...
app.post("/api/order", async (req, res) => {
  try {
    ...
    const orderResult = await binanceFetch("/fapi/v1/order","POST",body,true);
    // 수동 시장가 진입(청산성 아님)일 때만 알림
    if (body.type === "MARKET" && !(body.reduceOnly==="true"||body.reduceOnly===true) && !(orderResult.code && orderResult.code < 0)) {
      const dir = body.side === "BUY" ? "LONG" : "SHORT";
      notifyTelegram(`🟢 수동 진입 ${dir} ${sym}\n수량 ${body.quantity||"-"}\n${nowStr()}`);
    }
...
app.post("/api/telegram-test", (req, res) => {
  if (!TG_TOKEN || !TG_CHAT_ID) return res.status(400).json({ error: "TELEGRAM_TOKEN / TELEGRAM_CHAT_ID 미설정" });
  notifyTelegram(`✅ APEX TRADER 알림 테스트\n이 메시지가 보이면 알림이 정상 작동합니다.\n${nowStr()}`);
  res.json({ success: true, message: "테스트 메시지 전송됨 (텔레그램 확인)" });
});
...
app.post("/api/auto/config", (req, res) => {
  const { enabled, interval, symbols, leverage, marginUsdt, minConfidence, aiConfirm, aiGateScore, cooldownMin, aiStrict, aiMinConfidence, aiModel } = req.body;
  ...
  if(aiModel && ["claude-sonnet-4-6","claude-haiku-4-5","claude-opus-4-8"].includes(aiModel)) autoState.aiModel = aiModel;
  ...
  if(autoState.enabled) {
    ...
    addAutoLog(`🚀 자동매매 ON | 주기:${autoState.interval}s | 코인:${autoState.symbols.join(",")} | 레버리지:${autoState.leverage}x | 증거금:$${autoState.marginUsdt} | 쿨다운:${autoState.cooldownMin}분 | AI확인:${autoState.aiConfirm?'ON('+(autoState.aiModel==='claude-haiku-4-5'?'Haiku':autoState.aiModel==='claude-opus-4-8'?'Opus':'Sonnet')+',게이트'+autoState.aiGateScore+'점,'+(autoState.aiStrict?'strict':'관대')+')':'OFF'}`);
  } else {
    stopAutoTimer();
    addAutoLog("⏹ 자동매매 OFF");
  }
...
app.listen(PORT, () => {
  ...
  console.log(`   Telegram:   ${TG_TOKEN && TG_CHAT_ID ? "✅ SET (알림 활성)" : "⚠️ 미설정 (알림 비활성)"}`);
