...
async function runAutoTrade() {
  ...
  for (const symbol of autoState.symbols) {
    try {
      ...
      if(sig.stopLoss) {
        try {
          await binanceFetch("/fapi/v1/order","POST",{symbol,side:side==="BUY"?"SELL":"BUY",type:"STOP_MARKET",stopPrice:sig.stopLoss.toFixed(dec),closePosition:"true",positionSide:"BOTH",timeInForce:"GTE_GTC"},true);
          addAutoLog(`🛡 SL 설정: $${sig.stopLoss.toFixed(dec)}`);
        } catch(e){addAutoLog(`⚠️ SL 설정 실패: ${e.message}`);}
      }
      if(sig.takeProfit) {
        try {
          await binanceFetch("/fapi/v1/order","POST",{symbol,side:side==="BUY"?"SELL":"BUY",type:"TAKE_PROFIT_MARKET",stopPrice:sig.takeProfit.toFixed(dec),closePosition:"true",positionSide:"BOTH",timeInForce:"GTE_GTC"},true);
